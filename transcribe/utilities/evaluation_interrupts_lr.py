...
def detect_any_overlaps_turns111111(
    ...
    *,
    ...
    """
    Wersja oparta o detect_any_overlaps(...) z evaluation_interrupts.py. :contentReference[oaicite:4]{index=4}
    Zwraca listę overlap-okien z referencjami do segmentów AG/CL.
    """
    ...
    while i < len(ag_segs) and j < len(cl_segs):
        ...
        s = max(float(A["start"]), float(C["start"]))
        e = min(float(A["end"]), float(C["end"]))
        overlap = e - s
        ...
        # przesuwamy wskaźnik krótszego segmentu
        if float(A["end"]) < float(C["end"]):
            i += 1
        else:
            j += 1
...
def detect_starts_while_other_speaks_turns11111111(
    ...
    *,
    ...
    """
    Wersja oparta o detect_starts_while_other_speaks(...) z evaluation_interrupts.py. :contentReference[oaicite:5]{index=5}
    Zwraca eventy: starter zaczyna w trakcie wypowiedzi drugiej strony.
    """
    ...
    for o in while_segs:
        o_s, o_e, o_text = float(o["start"]), float(o["end"]), str(o.get("text", ""))
