    ...
    for i, s in enumerate(segs):
        ...
        if prev_s and next_s:
            ...
            if sandwich and close and shorty:
                ...
                fixed.append(Turn(start=s.start, end=s.end, role=prev_s.role, text=s.text))
                continue
...
def repair_diarized_segments_llm(
    ...
    *,
    ...
    def _call_one(chunk: List[Turn]) -> List[Turn]:
        user = (
...
def parse_diarized_segments(transcription: Any, speaker_map: Optional[Dict[str, str]] = None) -> List[Turn]:
    ...
    for s in segs:
        ...
        out.append(Turn(start=float(start), end=float(end), role=str(new_spk), text=text))
...
def parse_timestamped_segments(transcription: Any, speaker_id: str) -> List[Turn]:
    """
    ...
    for s in segs:
        ...
        if start is None or end is None:
            continue
        out.append(Turn(start=float(start), end=float(end), role=speaker_id, text=text))
    out.sort(key=lambda x: (x.start, x.end))
    return out
...
def transcribe_mono_audio_file_to_scenario_lr(
    *,
    source_file: str,
    temp_root_dir: str,
    metadata: Any = None,
