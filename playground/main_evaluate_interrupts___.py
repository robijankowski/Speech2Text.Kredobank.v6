...
def transcript_audio_file_verbose_o4_diarize(
    ...
    settings.USE_AZURE_OPENAI = "Y"  # force native for testing
...
def detect_agent_interruptions_from_diarized_segments(file_name: str) -> Optional[str]:
    try:
...
if __name__ == "__main__":
    ...
    for f in files:
        print(f"\n\n\n#########################\nFILE: '{f}'\n#########################")
        res = detect_agent_interruptions_from_diarized_segments( f )
        print("\nSTATS:", res)
